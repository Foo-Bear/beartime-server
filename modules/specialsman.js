...
var fs = require('fs')
  , Log = require('log')
  , log = new Log('debug', fs.createWriteStream('../specialsman.log'));
...
*/
var redis = new ioredis(config.dbport, config.dbaddr3);
...
redislistener.on('message', function (channel, message) {
  ...
  redis.get('specials', function (err, res) {
    ...
    if (IsJsonString(message)) {
      ...
      if (Array.isArray(specials) === false) {specials = []; log.debug("specials is bad, restarting");}
    ...
    } else {log.debug("message invalid json");}
  });
});
